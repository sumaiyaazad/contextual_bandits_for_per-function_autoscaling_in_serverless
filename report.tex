...
\subsection{State-of-the-Art Non-Bandit Approaches}

Beyond the bandit framework we adopt, the dominant paradigm for solving the serverless prewarming problem is \textit{predictive autoscaling} via time-series forecasting. The core idea is to forecast future request volumes and proactively scale instances before demand arrives, rather than learning a policy from reward feedback.

\textbf{Time-series forecasting methods.} Classical statistical models such as ARIMA and its seasonal variant SARIMA have been applied to predict serverless invocation patterns. Yemets~\cite{yemets2024} uses SARIMA to forecast request arrival times and prewarm containers accordingly, implementing a Prediction-Based Autoscaler (PBA) that outperforms Kubernetes' default Horizontal Pod Autoscaler (HPA). More recently, deep learning methods---particularly LSTM (Long Short-Term Memory) networks---have become the preferred approach. Suleiman et al.~\cite{suleiman2024} propose multi-step LSTM workload prediction for cloud autoscaling, and Kyrychenko et al.~\cite{kyrychenko2025} use AWS SageMaker's DeepAR model (an autoregressive LSTM variant) for predictive autoscaling of AWS Lambda functions via SQS queue metrics, reporting significant reductions in cold starts. Hybrid approaches combining Prophet with LSTM~\cite{flashserve2025} have also been proposed for serverless LLM inference, where cold starts are especially costly due to multi-gigabyte model loading.

\textbf{Platform-level solutions.} Major cloud providers have introduced built-in mitigations: AWS Lambda offers \textit{provisioned concurrency}, which keeps a user-specified number of instances permanently warm, and Azure Functions provides analogous pre-warmed instance pools. These are essentially static or manually-tuned approaches that guarantee a fixed baseline but cannot adapt to workload changes without human intervention.

\textbf{Infrastructure-level optimizations.} An orthogonal line of work reduces cold-start \textit{duration} rather than \textit{frequency}. Techniques include checkpoint/restore (CRIU-based snapshotting of initialized containers), fork-based replication that clones a warm parent process via \texttt{fork()} to achieve near-zero startup latency~\cite{chai2025}, container image de-duplication, and lightweight isolation via microVMs (e.g., AWS Firecracker). These approaches are complementary to prewarming policies: they reduce the cost of a cold start when one does occur, while our approach reduces how often cold starts happen in the first place.

\textbf{Comparison with our approach.} The key distinction is that time-series forecasting and platform-level solutions require explicit demand prediction or manual configuration, and their performance degrades under distribution shift---when the workload pattern changes unexpectedly, the forecast becomes unreliable and the system either over-provisions (wasting cost) or under-provisions (suffering cold starts). Our contextual bandit approach sidesteps demand forecasting entirely: instead of predicting \textit{how many requests will arrive}, the agent learns \textit{which prewarming level yields the best reward} given the current context. This makes it inherently more robust to non-stationarity, as demonstrated by our results on abrupt-shift and bursty workloads. However, time-series methods may have an advantage under highly regular, predictable workloads where accurate forecasts are possible, and infrastructure-level optimizations remain complementary to any prewarming strategy.
